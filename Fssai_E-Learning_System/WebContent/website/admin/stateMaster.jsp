...
function editState(){
	...
	//alert( document.getElementById('stateLabel').value);
...
function deleteState1() {
	var idHidden =  $("#idLabel1").val();
	var status = $("#statusLabel1").val();
	alert(idHidden + "  "+ status);
	document.getElementById('btnUpdate').style.display = 'none';
	document.getElementById('btnCreate').style.display = 'block';
	$(".displayNone").css("display","block");
		var total = "id="+idHidden+"&status="+ status+ "";
		...
		var stateName =  $("#stateName").val();
		 {
			...
			var j=1;
			$('#newTable tr').remove();
			$('#newTable').append('<tr  class="background-open-vacancies" style="background-color:#000077;"><th>S.No.</th><th>State Name</th><th>Status</th><th>Option</th><th style="display:none"></th></tr>')
			$.each(mainData1 , function(i , obj)
			{
				$('#newTable').append('<tr id="tableRow"><td>'+j++ +'</td><td><input type="hidden" id="stateLabel" value="'+obj.stateName+'">'+obj.stateName+'</td><td><input type="hidden" id="statusLabel" value="'+obj.status+'">'+obj.status+'</td><td> &nbsp;&nbsp;&nbsp;&nbsp; <a href="#" onClick="deleteState();">delete</a> </td><td style="display:none;"><input type="hidden" id="idLabel" value="'+obj.stateId+'"></td></tr>');	
			});
			}
			});
		return result;
